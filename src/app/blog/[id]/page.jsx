          ...
          <>
            {blogDetails.comments.map((comment) => (
              <div key={comment._id} className="flex gap-3 py-5 items-center">
                ...
                <div>
                  <p className="text-whiteColor text-sm font-bold ">{comment?.user?.name}</p>
                  <p className="text-red-600">{comment.text}</p>
                </div>

                {session?.user?._id === comment?.user?._id && (
                    <span className="text-red-700 p-5 m-5"> 
                <BsTrash onClick={() => handleDeleteComment(comment._id)} cursor="pointer" className="text-red-500 ml-10" /></span>
                )}
